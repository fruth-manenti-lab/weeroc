...
## External Hold Follow-Up: 5 ns Timing, Conversion Delay, and Peak Sensing

- A first attempted 5 ns scan was flat because the signal generator output channel was off. After restoring the generator output, the same coarse recheck recovered the expected injected Ctest response.
- Ran a finer external track-and-hold scan over `440..640 ns` in `5 ns` steps, with 50 acquisitions per point, `IO1` mux index `5`, Ctest enabled, `paT` gain code 1, threshold DAC 250, and 400 ns conversion delay:
...
/Users/tengiz/weeroc/.conda-radioroc/bin/python scripts/radioroc_standard_scurves.py \
  ...
  --sync-io-mux-index 5 \
  --adc-rstn-manual \
  --out-dir radioroc_runs/hold_external_ch4_ctest_sync_mux5_ultrafine_siggen_on_20260626
```

- The 5 ns scan resolved the timing shape:
  - HG rises from about 276 ADC units at 440 ns to about 744 at 460 ns.
  - HG reaches the high plateau by 465-475 ns and stays near 802-805 through about 555 ns.
  - HG rolls off after about 575 ns; by 640 ns it is down to about 316.
  - LG peaks later/lower, reaching about 285 around 525 ns.
  - A practical track-and-hold setting remains around 500-550 ns.

![Channel 4 external hold scan with 5 ns timing steps](assets/2026-06-26-external-holdscan-ch4-ctest-sync-mux5-5ns.png)

- Compared ADC conversion delay values of 80 ns, 400 ns, and 800 ns using the same external-hold Ctest setup. The scan shapes were effectively identical across this range, so conversion delay is not the limiting timing parameter for the present setup.
- Mapped the vendor external-hold peak-sensing option from `holdscan.pyc`: peak sensing changes FPGA control word `25` from `01111000` to `01111100`, preserves word `23` with prefix `01`, and uses external-hold ASIC setup. Added `--hold-peak-sensing` to `scripts/radioroc_standard_scurves.py` for this path.
- Ran the synchronized external-hold peak-sensing scan over `350..750 ns`:
...
/Users/tengiz/weeroc/.conda-radioroc/bin/python scripts/radioroc_standard_scurves.py \
  ...
  --hold-max-ns 750 \
  --hold-step-ns 25 \
  --hold-acquisitions 30 \
  --hold-conversion-delay-ns 160 \
  --pat-gain 1 \
  --use-ctest \
  --hold-synchro-trigger \
  --sync-io io1 \
  --sync-io-mux-index 5 \
  --adc-rstn-manual \
  --out-dir radioroc_runs/hold_external_peak_ch4_ctest_sync_mux5_20260626
```

- Peak-sensing result:
  - Baseline before 425 ns: HG/LG about 143/142 ADC units.
  - HG rises sharply at 450-475 ns.
  - Once the hold is later than the peaking time, HG remains high near 811-814 through 750 ns instead of falling with the shaper waveform.
  - LG similarly settles near 298 after about 525 ns.
- This matches the manual distinction: track-and-hold reconstructs the shaper waveform, while peak sensing samples the peak once the hold is sufficiently late.

![Channel 4 external hold peak-sensing scan with synchronized Ctest injection](assets/2026-06-26-external-peak-holdscan-ch4-ctest-sync-mux5.png)
