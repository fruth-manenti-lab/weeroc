...
from radioroc_client import DEFAULT_CONFIG, RadiorocDevice, RadiorocSerial, ScurveConfig, default_run_dir, parse_channels
...
def main() -> int:
    ...
    **Returns**
    ...
    try:
        connection.validate()
        operation = ScurveJobConfig(scan_config,
        ...
        preview = ScurveJob.preview(operation)
        ...
        def progress(event):
            if event.kind == "point":
                ...
                print(f"scurve dac={event.dac} values={[values[f'ch{ch}'] for ch in channels[:8]]}", flush=True)
        ...
        try:
            if args.verify_restoration:
                ...
                with RadiorocSerial.from_config(connection) as transport:
                    result = ScurveJob().run(RadiorocDevice(transport), operation, metadata=metadata,
                                             cancellation=cancellation, on_event=progress)
        finally:
            signal.signal(signal.SIGINT, previous_handler)
        print(f"S-curve {result.status}: {result.points} points; cleanup={result.cleanup_status}")
