"""Offline acceptance tests for opt-in S-curve restoration verification."""
...
from radioroc_client import RadiorocDevice, ScurveConfig, ScurveResult, bits
from radioroc.application import CancellationToken, JobBusyError
from radioroc.application.scurve import ScurveJob, ScurveJobConfig, _FPGA_SCURVE_ADDRESSES
...
class ScurveVerificationTransport(ScurveTransport):
    """ScurveTransport with faults that begin during independent verification."""

    def __init__(self, *, short_read=False, fail_read=False, fail_cleanup=False, mismatch=False,
                 point_readings=None):
        super().__init__(point_readings=point_readings)
    ...
    def write_word(self, address, value):
        ...
        # Word 6 is part of the job's own FPGA footprint and is fully restored
        # by scan cleanup before verification starts (it is the last address
        # restored, in _FPGA_SCURVE_ADDRESSES insertion order); drift word 1
        # afterward so only the verifier's independent re-read can notice.
        if self.mismatch and address == 6 and value == self.original_words[6]:
            self.words[1] = bits(200)

    def read_words(self, address, length):
        if address != 55:
            return super().read_words(address, length)
        # The job's own snapshot capture and the verifier's own re-read are
        # the only two ASIC FIFO reads of more than one byte in a run; count
        # only those so the faults below target the verifier specifically.
        if length > 1:
            self.read55_count += 1
...
class ScurveVerificationTests(unittest.TestCase):
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory()
        self.addCleanup(self.tmp.cleanup)
        self.base = ScurveConfig(
            channels=[4, 5], dac_min=0, dac_max=0, dac_step=1, clock_index=3,
            out_dir=Path(self.tmp.name) / "run",
        )

    def run_job(self, transport, config=None, **kwargs):
        operation = ScurveJobConfig(config or self.base)
        return ScurveJob().run(RadiorocDevice(transport), operation,
                               verify_restoration=True, **kwargs)

    def test_clean_run_captures_full_fpga_footprint_and_passes(self):
        transport = ScurveVerificationTransport()
        ...
        self.assertEqual(set(_row_keys(report["expected"]["fpga"])), set(_FPGA_SCURVE_ADDRESSES))
        self.assertEqual(set(_row_keys(report["observed"]["fpga_before_asic"])), set(_FPGA_SCURVE_ADDRESSES))
        self.assertEqual(set(_row_keys(report["observed"]["fpga_after_asic"])), set(_FPGA_SCURVE_ADDRESSES))
    ...
    def test_dac_registers_are_always_verified_for_t1_and_t2(self):
        for t1 in (True, False):
            with self.subTest(t1=t1):
                transport = ScurveVerificationTransport()
                config = replace(self.base, t1=t1, out_dir=Path(self.tmp.name) / f"t1-{t1}")
                ...
                self.assertIn((65, 1) if t1 else (65, 3), expected_keys)

    def test_cancelled_run_is_verified_after_scan_cleanup(self):
        transport = ScurveVerificationTransport()
        token = CancellationToken()
        config = replace(self.base, dac_max=1)
    ...
    def test_verification_keeps_the_transport_job_lock_until_it_finishes(self):
        ...
        with patch("radioroc.application.scurve.run_restoration_check", side_effect=blocked_verify):
            ...
            with self.assertRaises(JobBusyError):
                ScurveJob().run(RadiorocDevice(transport), ScurveJobConfig(second))
            release.set()
            thread.join(2)
    ...
    def test_mismatch_is_reported_without_repairing_scan_state(self):
        transport = ScurveVerificationTransport(mismatch=True)
        result = self.run_job(transport)
        report = result.verification
        self.assertEqual((result.status, result.cleanup_status, report["status"]),
                         ("completed", "restored", "failed"))
        self.assertTrue(any(item.get("address") == 1 for item in report["mismatches"]))
        self.assertFalse(any(item[:2] == ("write", 1) for item in transport.trace[-8:]))
    ...
    def test_short_read_is_incomplete_and_does_not_invent_values(self):
        transport = ScurveVerificationTransport(short_read=True)
        result = self.run_job(transport)
        report = result.verification
    ...
    def test_verifier_read_and_verifier_cleanup_errors_are_separate(self):
        transport = ScurveVerificationTransport(fail_read=True, fail_cleanup=True)
        result = self.run_job(transport)
        report = result.verification
    ...
    def test_cli_exit_codes_reflect_status_and_verification(self):
        ...
        for status, verification_status, expected_code in cases:
            with self.subTest(status=status, verification=verification_status):
                ...
                args = ["scurve", "--execute", "--verify-restoration",
                        "--out-dir", str(Path(self.tmp.name) / f"cli-{status}-{verification_status}")]
                with patch("sys.argv", args), \
                     patch.object(cli.RadiorocSerial, "from_config", return_value=ScurveVerificationTransport()), \
                     patch.object(cli.ScurveJob, "run", return_value=result), \
    ...
    def test_cli_dry_preview_with_verification_never_constructs_transport_or_output(self):
        ...
        args = ["scurve", "--verify-restoration", "--port", "/does/not/exist", "--out-dir", str(output_dir)]
        with patch("sys.argv", args), \
