"""Offline acceptance tests for opt-in hold-scan restoration verification."""
...
from radioroc_client import HoldScanConfig, HoldScanResult, RadiorocDevice, bits
from radioroc.application import CancellationToken, JobBusyError
from radioroc.application.hold_scan import HoldScanJob, HoldScanJobConfig, _FPGA_HOLD_ADDRESSES
...
from test_hold_scan_jobs import HoldTransport, adc_payload


class HoldVerificationTransport(HoldTransport):
    """HoldTransport with faults that begin during independent verification."""
    ...
    def write_word(self, address, value):
        ...
        # by scan cleanup before verification starts; drift it afterward so
        # only the verifier's independent re-read can notice.
        if self.mismatch and address == 78 and value == self.original_words[78]:
            self.words[26] = bits(200)

    def read_words(self, address, length):
        if address != 55:
            return super().read_words(address, length)
        # configure_adc_external_hold/internal_hold each read back ASIC 65/12
        # once per hold point (a one-byte read_words(55, 1) call) before the
        # job's own snapshot/verifier reads happen. Only count full
        # snapshot-sized reads so the fault below targets the verifier's own
        # readback, not one of those unrelated single-register reads.
        if length > 1:
            self.read55_count += 1
...
class HoldVerificationTests(unittest.TestCase):
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory()
        self.addCleanup(self.tmp.cleanup)
        self.base = HoldScanConfig(
            mode="external", channels=[4], trigger_channel=4,
            hold_min=0, hold_max=0, hold_step=5, acquisitions=2, timeout_s=1.0,
            out_dir=Path(self.tmp.name) / "run",
        )

    def run_job(self, transport, config=None, **kwargs):
        operation = HoldScanJobConfig(config or self.base)
        return HoldScanJob().run(RadiorocDevice(transport), operation,
                                 verify_restoration=True, **kwargs)

    def test_clean_run_captures_full_fpga_footprint_and_passes(self):
        transport = HoldVerificationTransport()
        ...
        self.assertEqual(set(_row_keys(report["expected"]["fpga"])), set(_FPGA_HOLD_ADDRESSES))
        self.assertEqual(set(_row_keys(report["observed"]["fpga_before_asic"])), set(_FPGA_HOLD_ADDRESSES))
        self.assertEqual(set(_row_keys(report["observed"]["fpga_after_asic"])), set(_FPGA_HOLD_ADDRESSES))
    ...
    def test_cancelled_run_is_verified_after_scan_cleanup(self):
        ...
        config = replace(self.base, hold_max=5)
    ...
    def test_verification_keeps_the_transport_job_lock_until_it_finishes(self):
        ...
        with patch("radioroc.application.hold_scan.run_restoration_check", side_effect=blocked_verify):
            ...
            with self.assertRaises(JobBusyError):
                HoldScanJob().run(RadiorocDevice(transport), HoldScanJobConfig(second))
            release.set()
            thread.join(2)
    ...
    def test_mismatch_is_reported_without_repairing_scan_state(self):
        transport = HoldVerificationTransport(mismatch=True)
        result = self.run_job(transport)
        report = result.verification
        self.assertEqual((result.status, result.cleanup_status, report["status"]),
    ...
    def test_short_read_is_incomplete_and_does_not_invent_values(self):
        transport = HoldVerificationTransport(short_read=True)
        result = self.run_job(transport)
        report = result.verification
    ...
    def test_verifier_read_and_verifier_cleanup_errors_are_separate(self):
        transport = HoldVerificationTransport(fail_read=True, fail_cleanup=True)
        result = self.run_job(transport)
        report = result.verification
    ...
    def test_cli_exit_codes_reflect_status_and_verification(self):
        ...
        for status, verification_status, expected_code in cases:
            with self.subTest(status=status, verification=verification_status):
                result = HoldScanResult(
                    csv_path=Path(self.tmp.name) / f"{status}-{verification_status}.csv",
                    status=status,
                    verification={"status": verification_status},
                )
                args = ["hold-scan", "--execute", "--verify-restoration",
                        "--out-dir", str(Path(self.tmp.name) / f"cli-{status}-{verification_status}")]
                with patch("sys.argv", args), \
                     patch.object(cli.RadiorocSerial, "from_config", return_value=HoldTransport()), \
                     patch.object(cli.HoldScanJob, "run", return_value=result), \
    ...
    def test_cli_dry_preview_with_verification_never_constructs_transport_or_output(self):
        ...
        args = ["hold-scan", "--verify-restoration", "--port", "/does/not/exist", "--out-dir", str(output_dir)]
        with patch("sys.argv", args), \
    ...
    def test_threshold_dac_registers_are_verified_after_fix(self):
        """With the registers() fix, a scan that sets threshold_dac captures
        and verifies ASIC 65/1 and 65/2 as part of the same restoration check
        used for every other temporary register."""
        transport = HoldVerificationTransport()
