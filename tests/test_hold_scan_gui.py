...
Mirrors tests/test_threshold_gui.py; see that module for the design rationale
of each scenario.
"""
...
class HoldScanGuiTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def setUp(self):
        ...
        self.window.hold_max.setValue(20)
        self.window.hold_step.setValue(5)
        self.window.acquisitions.setValue(2)
    ...
    def test_configure_finish_plot_reopen(self):
        ...
        self.window.set_threshold_dac.setChecked(True)
        self.window.threshold_dac.setValue(200)
        self.window.start_run()
        ...
        self.assertGreater(len(self.window.axes.lines) + len(self.window.axes.containers), 0)
        manifest = json.loads((self.directory / "metadata.json").read_text())
        self.assertFalse(manifest["operation"]["scan"]["t1"])
        self.assertEqual(manifest["simulation"]["baseline_counts"], 125)
    ...
    def test_cancel_close_keeps_event_loop_responsive_and_waits_for_cleanup(self):
        ...
        from radioroc.transport.hold_scan_simulator import create_hold_simulator

        class SlowSession:
            """Add a small real delay per acquisition so the run stays
            observably 'running' long enough for this test to reliably
            catch it and to close mid-flight."""
        ...
        self.window.worker_factory = lambda op, sim: HoldScanWorker(op, sim, session_factory=SlowSession)
        ticks = []
        heartbeat = QTimer()
        ...
        heartbeat.start()
        self.window.show()
        self.window.hold_max.setValue(100)
        self.window.hold_step.setValue(5)
        self.window.start_run()
    ...
    def test_close_failure_remains_visible(self):
        ...
        self.window.worker_factory = lambda op, sim: HoldScanWorker(op, sim, session_factory=BrokenClose)
        self.window.show()
        self.window.start_run()
        self.window.close()
        self.wait_idle()
