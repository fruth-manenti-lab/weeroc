"""S-curve controls and presentation; measurement belongs to the shared job.

This mirrors ``radioroc.gui.hold_scan_window.HoldScanWindow`` field-for-field
...
that module for the design rationale of each piece. S-curve's own config
surface is smaller than hold scan's: no hold-mode/sync-IO/ADC-timing/gain-
override groups, and the plot is a single turn-on-percentage value per
channel per DAC point rather than hg/lg mean+stdev error bars.
"""
...
class ScurveWindow(QMainWindow):
    ...
    def __init__(self, *, worker_factory=ScurveWorker,
        ...
        title = QLabel("RADIOROC  |  S-curve workflow")
        ...
        self.dac_max = _integer(0, 1023, 1023)
        self.dac_step = _integer(1, 1023, 50)
        ...
        for label, field in [("First DAC code", self.dac_min),
                             ("Last DAC code", self.dac_max),
                             ("DAC step", self.dac_step),
                             ("S-curve clock index (0..3)", self.clock_index),
        ...
        for field in (self.trigger_level, self.mask, self.ctest, self.initialize, self.defaults):
            form.addRow(field)
        ...
        self.sim_group = QGroupBox("Synthetic turn-on curve (borrows the validated threshold logistic model)")
        ...
        self.width = _decimal(0.001, 1023, 30)
        self.channel_spacing = _decimal(-1023, 1023, 3)
        self.pulse_count = _integer(0, 255, 200)
        for label, field in [("Midpoint (DAC code)", self.midpoint), ("Width (DAC codes)", self.width),
                             ("Channel spacing (DAC codes)", self.channel_spacing),
                             ("Pulse count per point", self.pulse_count)]:
    ...
    def _mode_changed(self):
        ...
        self.run_button.setText("Run simulation" if simulation else "Run hardware S-curve scan")
        self.sim_group.setEnabled(simulation and self.worker is None)
        self._update_connection_controls()

    def _show_mode_banner(self):
        self.banner.setText(
            "SIMULATION · Synthetic turn-on percentages, not measured lab data. No board connection."
    ...
    def poll_connection_worker(self):
        ...
        # behind a scan.  Consume its terminal S-curve snapshot before
    ...
    def operation(self):
        ...
        scan = ScurveConfig(
            channels=channels, dac_min=self.dac_min.value(), dac_max=self.dac_max.value(),
            dac_step=self.dac_step.value(), t1=self.discriminator.currentIndex() == 0,
        ...
        return ScurveJobConfig(scan, Path(config).expanduser() if config else None,
                               self.initialize.isChecked(), self.defaults.isChecked())

    def simulation(self):
        settings = ScurveSimulationConfig(
            midpoint=self.midpoint.value(), width=self.width.value(),
            channel_spacing=self.channel_spacing.value(), pulse_count=self.pulse_count.value(),
        )
        settings.validate()
        return settings
    ...
    def preview(self):
        try:
            ...
            data = ScurveJob.preview(operation)
            simulation = self.mode.currentIndex() == 0
            ...
            if simulation:
                ...
                data["hardware"] = {
                    ...
                    "scurve_available": (self.connection_worker is not None and
    ...
    def cancel_run(self):
        if self.worker:
            ...
            try:
                self.connection_worker.cancel_scurve()
            except Exception:
                return
        else:
            return
    ...
    def poll_worker(self):
        if self.worker is not None:
            ...
            snapshot = self.connection_worker.scurve_snapshot()
            mode = "HARDWARE"
        ...
        if snapshot.rows != self._last_rows:
            ...
            title = "SIMULATION · synthetic S-curve response" if mode == "SIMULATION" else "HARDWARE · live S-curve response"
    ...
    def choose_saved(self):
        path, _ = QFileDialog.getOpenFileName(self, "Open S-curve result", "",
                                             "S-curve results (metadata.json scurve.csv);;CSV (*.csv)")
        if path:
    ...
    def open_saved(self, path):
        if self.worker is not None or self._hardware_running:
            return
        try:
            saved = read_scurve_run(Path(path))
