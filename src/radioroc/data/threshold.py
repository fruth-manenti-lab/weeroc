...
class ThresholdRunWriter:
    def __init__(self, directory: Path, channels: list[int], manifest: dict):
        self.directory = Path(directory)
        self.metadata_path = self.directory / "metadata.json"
        self.csv_path = self.directory / "thresholdscan.csv"
        self.attempts_path = self.directory / "thresholdscan_attempts.csv"
        self.point_fields = ["DAC", *(f"ch{ch}" for ch in channels)]
        self.attempt_fields = ["DAC", "channel", "attempt", "rate_hz", "trigger_count"]
        self.directory.mkdir(parents=True, exist_ok=True)
        for path in (self.metadata_path, self.csv_path, self.attempts_path):
            if path.exists():
        ...
        self._header(self.attempts_path, self.attempt_fields)
    ...
    def append_attempt(self, row: dict) -> None:
        self._append(self.attempts_path, self.attempt_fields, row)

    def append_point(self, row: dict) -> None:
        self._append(self.csv_path, self.point_fields, row)
